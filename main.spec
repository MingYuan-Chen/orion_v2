# -*- mode: python ; coding: utf-8 -*-

a = Analysis(
    ['main.py'],
    pathex=['.'],
    binaries=[],
    datas=[
        ('resources/icons/header.png', 'resources/icons'), 
        ('resources/icons/header.ico', 'resources/icons'),
        ('resources/commands/common', 'resources/commands/common'),
        ('resources/commands/hydra_fhd', 'resources/commands/hydra_fhd'),
        ('resources/commands/hydra', 'resources/commands/hydra'),
        ('resources/commands/gemini_fhd', 'resources/commands/gemini_fhd'),
        ('resources/commands/gemini', 'resources/commands/gemini'),
        ('resources/commands/argo', 'resources/commands/argo'),
        ('resources/commands/athena', 'resources/commands/athena'),
        ('gui/ui/main_window.ui', 'gui/ui'),
        ('gui/ui/login_dialog.ui', 'gui/ui'),
        ('gui/ui/device_manager_widget.ui', 'gui/ui'),
        ('gui/ui/device_connection_dialog.ui', 'gui/ui')
        ],
    hiddenimports=[],
    hookspath=[],
    hooksconfig={},
    runtime_hooks=[],
    excludes=[],
    noarchive=False,
    optimize=0,
)
pyz = PYZ(a.pure)

exe = EXE(
    pyz,
    a.scripts,
    a.binaries,
    a.datas,
    [],
    name='vt_hydra_monitor',
    debug=False,
    bootloader_ignore_signals=False,
    strip=False,
    upx=True,
    upx_exclude=[],
    runtime_tmpdir=None,
    console=False,
    disable_windowed_traceback=False,
    argv_emulation=False,
    target_arch=None,
    codesign_identity=None,
    entitlements_file=None,
    icon='resources/icons/header.ico',
    version='version.rc',
)
