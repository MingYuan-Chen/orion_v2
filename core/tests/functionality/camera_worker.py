    ...
    def prepare_test_steps(self) -> List[TestStep]:
        ...
        if self.platform_name == "athena":
            return [
                TestStep(
                command=commands[0],
                description="Check camera display: Scorpius",
                criteria="Camera can displayt normally",
                pre_condition="Please Plug-in Scorpius camera",
                post_check="Is the Scorpius camera display normally"
            ),
            TestStep(
                command=commands[1],
                description="Check camera display: MIPI",
                criteria="Camera can displayt normally",
                pre_condition="Please Plug-in MIPI camera",
                post_check="Is the MIPI camera display normally"
            ),
            TestStep(
                command=commands[2],
                description="Check camera display: BFlex",
                criteria="Camera can displayt normally",
                pre_condition="Please Plug-in BFlex camera",
                post_check="Is the BFlex camera display normally"
            ),
            TestStep(
                command=commands[3],
                description="Check camera display: LVDS",
                criteria="Camera can displayt normally",
                pre_condition="Please Plug-in LVDS camera",
                post_check="Is the LVDS camera display normally"
            )
            ]
        else:
            return [
